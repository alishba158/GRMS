...
def admin_dashboard(request):
    ...
    # Try to get available supervisors
    try:
    ...
    context = {
        ...
        'pending_extensions': 0,  # Set to 0 if model doesn't exist
        'urgent_extensions': 0,   # Set to 0 if model doesn't exist
        'pending_degree_requests': 0,  # Set to 0 if model doesn't exist
...
def supervisor_dashboard(request):
    # Fix 1: Safe supervisor access with error handling
    try:
        supervisor = Supervisor.objects.get(user=request.user)
    except Supervisor.DoesNotExist:
        # User is not a supervisor, redirect to appropriate page
        return redirect('admin_dashboard')
    
    # Fix 2: Get students (handle if no students exist)
    students = Student.objects.filter(supervisor=supervisor)
    
    # Fix 3: Get unique departments (handle empty queryset)
    supervisor_departments = students.values_list('department', flat=True).distinct()
    
    # Fix 4: Handle department filter
    ...
    # Fix 6: Safe data fetching with error handling for all queries
    try:
    ...
    try:
        recent_extensions = ExtensionRequest.objects.filter(student__supervisor=supervisor, status='Pending').order_by('-requested_on')[:5]
    except:
        recent_extensions = []
    
    try:
        recent_theses = Thesis.objects.filter(student__supervisor=supervisor).order_by('-submitted_at')[:5]
    except:
        recent_theses = []
    
    # Fix 7: Create context with all safe values
    context = {
        ...
        if form.is_valid():
            ...
            if student.supervisor:
                Notification.objects.create(
                 ...
                 # 🔴🔴🔴 YAHAN PE YEH EMAIL CODE ADD KARO 🔴🔴🔴
